 ...
 * | --- | --- | --- | --- |
 * | Dec 05, 2013 | 1.0 | armfly | 首版 |
 * | Jun 20, 2014 | 1.1 | armfly | 增加大小端整数转换函数 |
 * | Apr 06, 2015 | 1.2 | armfly | 增加 BEBufToUint32()和 LEBufToUint32() |
 * | Oct 09, 2015 | 1.3 | armfly | 增加 BcdToChar(), HexToAscll()和 AsciiToUint32() |
 * | Oct 09, 2015 | 1.3a | armfly | 解决 HexToAscll() 函数末尾补0的BUG |
...
uint32_t ASCIITOHEX(uint8_t *buf, uint8_t len)
{
    ...
    {
        hex = (buf[0] - 0x30)*10 + (buf[1] - 0x30);
    }
    else if(len == 3)
    {
        hex = (buf[0] - 0x30)*100 + (buf[1] - 0x30)*10 + (buf[2] - 0x30);
    }
    else if(len == 4)
    {
        hex = (buf[0] - 0x30)*1000 + (buf[1] - 0x30)*100 + (buf[2] - 0x30)*10 + (buf[3] - 0x30);
    }
    else if(len == 5)
    {
        hex = (buf[0] - 0x30)*10000 + (buf[1] - 0x30)*1000 + (buf[2] - 0x30)*100 + (buf[3] - 0x30)*10 + (buf[4] - 0x30);
    }
    else if(len == 6)
    {
        hex = (buf[0] - 0x30)*100000 + (buf[1] - 0x30)*10000 + (buf[2] - 0x30)*1000 + (buf[3] - 0x30)*100 + (buf[4] - 0x30)*10 + (buf[5] - 0x30);
    }
...
void sys_invert_uint8(unsigned char *DesBuf, unsigned char *SrcBuf)
{
    int i;
...
void sys_invert_uint16(unsigned short *DesBuf, unsigned short *SrcBuf)
{
    int i;
...
uint16_t sys_crc16(unsigned char *puchMsg, unsigned int usDataLen)
{
    ...
    {
        wChar = *(puchMsg++);
        sys_invert_uint8((uint8_t *)&wChar, (uint8_t *)&wChar);
        wCRCin ^= (wChar << 8);
    ...
    sys_invert_uint16(&wCRCin, &wCRCin);
    crc16 = wCRCin;
    return crc16;
}
