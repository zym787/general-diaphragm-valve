 ...
 * | Jan 14, 2026 | 1.9 | Drinkto | 移植到stm32f030 |
 */
...
static void bsp_InitSoftTimer(void);
...
{
    /* 清零所有的软件定时器 */
    bsp_InitSoftTimer();    //初始化软件定时器

    /*
     配置systic中断周期为1ms，并启动systick中断。

     SystemCoreClock 是固件中定义的系统内核时钟，对于STM32F429,一般为 168/180MHz
 ...
 *   功能说明: 检测定时器是否开启
 *   形    参:     _id     : 定时器ID，值域【0,TMR_COUNT-1】。用户必须自行维护定时器ID，以避免定时器ID冲突。
 *   返 回 值: 返回 0 表示未开启， 1表示开启， FF表示定时器ID超出范围
 *******************************************************************************
 */
...
/*
 *******************************************************************************
 *   函 数 名: bsp_GetSystickISRFlag
 *   功能说明: 返回g_ucEnableSystickISR的值
 *             以判断是否执行bsp库的滴答定时中断服务程序
 *   形    参:  无
 *   返 回 值: g_ucEnableSystickISR的值
 *******************************************************************************
 */
uint8_t bsp_GetSystickISRFlag(void) {
    return g_ucEnableSystickISR;
}
/*
 *******************************************************************************
 *   函 数 名: bsp_SetSystickISRFlag
 *   功能说明: 设置g_ucEnableSystickISR的值
 *   形    参:  _switch 0 不执行bsp库的滴答定时中断服务程序
 *                      1 执行bsp库的滴答定时中断服务程序
 *   返 回 值: 无
 *******************************************************************************
 */
void bsp_SetSystickISRFlag(uint8_t _switch) {
    g_ucEnableSystickISR = ((_switch == TRUE) ? TRUE : FALSE);
}
