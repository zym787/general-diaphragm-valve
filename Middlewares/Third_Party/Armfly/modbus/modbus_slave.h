...
#include "modbus_address.h" ///添加寄存器表

#define SADDR485	1           ///mb地址
#define SBAUD485	UART1_BAUD  ///mb波特率

#define SBROADCAST_ADDR 0
#define SMB_ADDR_MIN    1
#define SMB_ADDR_MAX    247

///是否在modbus_address.h中定义寄存器表
...
{
    /* 03H 06H 读写保持寄存器 */
    uint16_t P00_INIT;  //初始化标志
    uint16_t P01_DIR;   //通道方向
    uint16_t P02_SPD;   //转速
    uint16_t P03_FACY;  //恢复出厂参数
    uint16_t P04_RCNT;  //当前次数
    uint16_t P05_RCNTK; //当前次数(万)
    uint16_t P06_STAR_STOP;
